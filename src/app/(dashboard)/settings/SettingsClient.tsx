...
export default function SettingsClient({ initialProfile, initialEmail }: Props) {
  const {
    user,
    setUser,
    showToast,
  ...
  // Source of truth for display: prefer the live store, fall back to SSR data.
  // This makes the form pre-populated on first render and stays in sync after
  // edits/saves.
  const display = user || initialProfile;
  const displayEmail = display?.email || initialEmail;

  const [tab, setTab] = useState<Tab>("profile");

  // Profile fields — initialized from the SSR profile so they're filled on the
  // very first render. No spinner, no empty flash.
  const [name, setName] = useState<string>(initialProfile?.full_name || "");
  const [preferredName, setPreferredName] = useState<string>(initialProfile?.preferred_name || "");
  const [phone, setPhone] = useState<string>(initialProfile?.phone || "");
  const [smsOptIn, setSmsOptIn] = useState<boolean>(!!initialProfile?.phone_sms_opt_in);
  const [city, setCity] = useState<string>(initialProfile?.city || "");
  const [zip, setZip] = useState<string>(initialProfile?.zip || "");
  const [householdSize, setHouseholdSize] = useState<number | "">(initialProfile?.household_size ?? "");
  const [childrenCount, setChildrenCount] = useState<number | "">(initialProfile?.children_count ?? "");
  const [countryOfOrigin, setCountryOfOrigin] = useState<string>(initialProfile?.country_of_origin || "");
  const [yearsInUs, setYearsInUs] = useState<number | "">(initialProfile?.years_in_us ?? "");
  const [languagesSpoken, setLanguagesSpoken] = useState<string[]>(initialProfile?.languages_spoken || []);

  const [language, setLanguage] = useState<string>(initialProfile?.preferred_language || "en");
  const [needs, setNeeds] = useState<string[]>(initialProfile?.needs || []);

  const [notif, setNotif] = useState<Required<NotificationPrefs>>({
    ...DEFAULT_NOTIF,
    ...(initialProfile?.notification_prefs || {}),
  });
  const [privacy, setPrivacy] = useState<Required<PrivacyPrefs>>({
    ...DEFAULT_PRIVACY,
    ...(initialProfile?.privacy_prefs || {}),
  });

  const [deleteOpen, setDeleteOpen] = useState(false);
  const [deleting, setDeleting] = useState(false);
  const [newEmail, setNewEmail] = useState("");
  const [paused, setPaused] = useState<boolean>(!!initialProfile?.is_paused);
  const [sendingTest, setSendingTest] = useState(false);
  const [saving, setSaving] = useState(false);

  // Push the SSR profile into Zustand on mount so other components (Sidebar,
  // Header) see the user immediately.
  useEffect(() => {
    if (initialProfile && !user) setUser(initialProfile);
  }, [initialProfile, user, setUser]);

  // Re-sync local form state when the store user changes (e.g., after a save).
  useEffect(() => {
    if (!user) return;
  ...
  const save = async (payload: Record<string, any>) => {
    const userId = user?.id || initialProfile?.id;
    if (!userId) return;
    setSaving(true);
  ...
  const onDownload = async () => {
    ...
    const blob = new Blob(
      [JSON.stringify({ profile: display, saved, notifications: notifs }, null, 2)],
      ...
      {tab === "profile" && (
        <div className="space-y-4">
          <section className="bg-white rounded-card border border-border p-6 space-y-4">
            <div className="flex items-center gap-4">
              <div className="h-16 w-16 rounded-full bg-primary-light text-primary-dark grid place-items-center font-semibold text-xl overflow-hidden">
                {display?.avatar_url ? (
                  ...
                  <img src={display.avatar_url} alt="" className="h-full w-full object-cover" />
                ) : (
                  (preferredName || name || display?.full_name || "U")[0]?.toUpperCase()
                )}
              </div>
            ...
            />
            <Input label={t("auth.email")} value={displayEmail} disabled />
            <Input
